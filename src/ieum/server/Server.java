                ...
                while (true) {
                    ...
                    switch (protocol) {
                        case Fun.LOGIN: {
                            ...
                            messageAll(Fun.LOGIN + "|" + id + "|" + name + "|" + sex + "|" + pos);
                            // 접속한 사람의 정보를 저장
                            waitVc.add(this);
                            // Client client=new Client();
                            messageTo(Fun.MYLOG + "|" + id);
                            // 로그인창에서 => 대기창으로 변경
                            for (Client user : waitVc) {
                                messageTo(
                                        Fun.LOGIN + "|" + user.id + "|" + user.name + "|" + user.sex + "|" + user.pos);
                            }
                            // 입장 메세지 출력
                            messageAll(Fun.WAITCHAT+"|[☞"+name+"님이 입장하셨습니다.");
                            // 개설된 방정보 전송
                        }
                        break;
                        case Fun.WAITCHAT:
                        {
                            messageAll(Fun.WAITCHAT+"|["+name+"]"+st.nextToken());
                        }
                    }
                }
            } catch (Exception ex) {
